...
function marcarError(element,msg){
    ...
    const small = document.getElementById("error"+element.id.charAt(0).toUpperCase()+element.id.slice(1));
    if(small){
...
function validar({nombre, correo, terminos}){
    ...
    const patron = new RegExp(emailInput.getAttribute("pattern"));
    if(!correo || !patron.test(correo)){
        marcarError(emailInput,"Correo no válido");
        ok= false;
    }

    if(!terminos){
        marcarError(document.getElementById("terminos"),"Debes aceptar los terminos");
        ok= false;
    }
...
function renderTabla(){
    ...
    lista.forEach((ins,i) =>{
        ...
        tr.innerHTML=`
            ...
            <td>${ins.correo}</td>
            <td>${ins.tipo}</td>
            <td><button type="button" class="btnEliminar" data-i="${i}">Eliminar</button></td>
        `;
